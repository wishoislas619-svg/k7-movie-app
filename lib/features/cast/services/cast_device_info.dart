...
class CastDeviceInfo {
  ...
  static CastDeviceType _detectType(CastDevice device) {
    ...
    final metadataText = device.metadata.entries
        .map((e) => '${e.key}:${e.value}')
        .join(' ')
        .toLowerCase();
    if (device.metadata['roku'] == 'true' ||
        nameLower.contains('roku') ||
        metadataText.contains('roku')) {
      return CastDeviceType.roku;
    }
    if (device.protocol == CastProtocol.chromecast) {
      if (nameLower.contains('chromecast')) return CastDeviceType.chromecast;
      if (nameLower.contains('android tv') ||
          nameLower.contains('shield') ||
          nameLower.contains('fire tv')) {
    ...
    if (device.protocol == CastProtocol.dlna) {
      ...
      if (nameLower.contains('android') || nameLower.contains('tv box'))
        return CastDeviceType.androidTv;
  ...
  IconData get icon {
    switch (deviceType) {
      case CastDeviceType.chromecast:
        return Icons.cast;
      case CastDeviceType.samsung:
        return Icons.tv;
      case CastDeviceType.lg:
      ...
      case CastDeviceType.dlnaGeneric:
        return Icons.tv;
      default:
  ...
  Color get accentColor {
    switch (deviceType) {
      ...
      case CastDeviceType.lg:
        return const Color(0xFFC00000); // LG Red
      case CastDeviceType.roku:
        return const Color(0xFF6C2BD9); // Roku Purple
      case CastDeviceType.appleTv:
        return const Color(0xFF9E9E9E); // Apple Silver
      case CastDeviceType.androidTv:
        return const Color(0xFF3DDC84); // Android Green
      case CastDeviceType.dlnaGeneric:
        return const Color(0xFF00A3FF); // K7 Blue
      default:
  ...
  String get protocolLabel {
    if (deviceType == CastDeviceType.roku) return 'Roku';
    switch (protocol) {
